...
function evalPath(segs, ctx) {
  ...
  // 确定起点与起始段
  let base, start;
  if (head === "root") { base = ctx.root; start = 1; }
  else if (head === "parent") { base = ctx.parent; start = 1; }
  else if (head === "children") {                    // 向后兼容：self.__children 向量
    base = (ctx.self && ctx.self.__children) ? ctx.self.__children : [];
    start = 1;
  } else { base = ctx.self; start = 0; }             // 无前缀 → self 字段 / 命名子集合
  // 逐段游走；遇数组（命名子集合）→ 映射为向量（支持任意深度的"按集合名聚合"）
  let v = base;
  for (let i = start; i < segs.length; i++) {
    if (v == null) return null;
    if (Array.isArray(v)) v = v.map((x) => (x == null ? null : x[segs[i]]));
    else v = v[segs[i]];                              // Proxy get：追踪依赖，pending 会抛出
  }
