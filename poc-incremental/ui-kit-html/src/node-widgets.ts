// 自定义节点组件注册表（HTML 端）：把一个模型子树（如 CustomerParty 槽位）交给宿主自定义组件渲染。
//   与 React 端 node-widgets 同构，只是组件返回 HTMLElement 而非 ReactNode。
//   PageDef 里 panel 设 widget:'name'；宿主启动时 registerNodeWidget('name', 组件)。
//   组件拿到 { node: PanelUI(含 children/nodePath/widgetProps), ctx }，自管展示与行为——
//   值永远经 ctx 读写引擎，children 用 renderUINode 渲染；未注册的 widget → egPanel 降级为默认 panel。
import type { EngineCtx, PanelUI } from '@udsl/ui-kit-core';

export type NodeWidget = (p: { node: PanelUI; ctx: EngineCtx }) => HTMLElement;
