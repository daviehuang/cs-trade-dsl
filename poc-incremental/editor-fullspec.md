...
- 装配 `uses`：`use`(选模块，裸名 / `alias.mod`) + `on`(宿主类型) + `as` + **`bind`(入参←宿主 expr)** + **`produce`(输出→宿主字段)**——可视化端口连线。（全缺）
...
- **Phase 2 · 模块/数据源 + 治理 + 分层 UI**：域 3 模块+uses 连线 · 域 4 dataSources+resolver 模拟 · 域 11 版本/diff/发布-回滚 · 域 14 分层 UI/模板向导。
